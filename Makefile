...
REDIRECT_SRC = $(addprefix $(SRC_DIR)executor/redirections/, redirects.c redirect_utils.c)
BUILTIN_SRC = $(addprefix $(SRC_DIR)executor/builtins/, echo.c pwd.c env.c export.x export_utils.c unset.c cd.c cd_utils.c)

SRC = $(SRC_FILES) $(UTILS_SRC) $(TOKENIZER_SRC) $(LEXER_SRC) $(PARSER_SRC) $(EXPANDER_SRC) $(EXECUTOR_SRC) $(REDIRECT_SRC) $(BUILTIN_SRC)
